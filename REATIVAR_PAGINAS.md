...
4. **No arquivo `src/pages/DashboardV02.tsx`**:
   - Localizar a linha comentada que contém `{/* TEMPORÁRIO: Campo de rotinas diárias oculto... */}` e `<DailyRoutine />`
   - Descomentar a linha `<DailyRoutine />` para reativar o campo de rotinas diárias
   - Remover o comentário `{/* TEMPORÁRIO: Campo de rotinas diárias oculto... */}`

5. **Páginas a reativar**:
   - Café (`/coffee`)
   - Expansão (`/expansion`)
...
6. **Pop-ups e campos a reativar**:
   - Campo de Rotina de hoje (card completo com profetização e agradecimentos)
