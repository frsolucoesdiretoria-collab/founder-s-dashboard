...
// Otimização Mobile: Carregamento dinâmico para evitar bloqueio do LCP - Mantido conforme solicitação do usuário
import { lazy } from 'react';

// Componentes Eager (Restaurando visualização completa - Emergência)
// Otimização Mobile: Carregamento dinâmico para evitar bloqueio do LCP - Mantido conforme solicitação do usuário
// import dynamic from 'next/dynamic'; // Vite incompatible

const BackgroundEffectsLazy = lazy(() => import('./components/BackgroundEffects'));
...
export default function AxisV535Page() {
    useEffect(() => {
        ...
        // Fase 3: Adicionado quasisolutions.com para otimizar CSS externo
        const preconnects = [
        ...
        // Fase 3: Expandido para incluir mais domínios externos
        const dnsPrefetch = [
        ...
        // Fase 2.1: Ajustado href para hero.webp (desktop) ao invés de hero-medium.webp
        // PageSpeed Insights testa principalmente desktop, então preload deve corresponder à versão desktop
        const preloadLink = document.createElement('link');
        preloadLink.rel = 'preload';
        preloadLink.as = 'image';
        preloadLink.href = `${ASSET_PREFIX}/images/imagens%20v5-3-4/hero.webp`;
        // @ts-ignore - imagesrcset não é TypeScript mas é válido HTML
        ...
        //  3. Carregar Google Fonts com font-display: swap (elimina FOIT - Flash of Invisible Text)
        ...
        // 4. CSS Não-Crítico com Lazy Loading (media=print trick - não bloqueia renderização)
        ...
        nonCriticalCSS.forEach(href => {
            ...
            link.onload = function () {
                (this as HTMLLinkElement).media = 'all'; // Ativa após carregar
            };
        ...
        // 5. Carregar Scripts com defer (não bloqueante, mantém ordem)
        const loadScripts = () => {
            const scriptFiles = [
            ...
            scriptFiles.forEach(src => {
                ...
                script.src = src;
                script.defer = true; // Defer: não bloqueia parsing, executa em ordem
                script.onerror = () => console.warn(`Failed to load script: ${src}`);
                document.body.appendChild(script);
            });
        };

        // 6. Carregar scripts usando Intersection Observer (just-in-time loading)
        // Aguarda Hero Section estar 70% visível antes de carregar scripts
        ...
        // Fallback: se não encontrar elemento, carrega após delay reduzido
        const heroElement = document.querySelector('#uw-root-container main');
        if (heroElement) {
            observer.observe(heroElement);
        } else {
            setTimeout(loadScripts, 800); // Reduzido de 1500ms para 800ms
        }
        ...
        <>
            ...
            <div
                ...
                style={{
                    ...
                    backgroundColor: '#000',
                    overflowX: 'hidden',
                    fontFamily: "'Futura Md BT', sans-serif" // Global font reset
                }}
            >
                <main>
                ...
                {/* Floating WhatsApp button - Overridden by user request */}
                <a
