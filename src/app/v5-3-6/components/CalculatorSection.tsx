...
export default function CalculatorSection() {
    ...
    const handleSubmitCta = async (e: React.FormEvent) => {
        ...
        try {
            // Envio para API route com campos do Notion
            const response = await fetch("/api/calculator-submit", {
                ...
                body: JSON.stringify({
                    ...
                    whatsapp: leadPhone,
                    ticketMedio: ticketVal,
                    consultasDia: Number(consultsPerDay) || 0,
                    cancelamentosMes: cancellationsVal,
                    perdaMensal: monthlyLoss,
            ...
            if (!response.ok) {
                console.warn("Failed to submit lead to API");
            }
        } catch (err) {
