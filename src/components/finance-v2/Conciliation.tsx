...
export function Conciliation() {
  const { accountPlans, costCenters } = useFinanceConfig();
  ...
  const getAccountPlanName = (id: string) => {
    return accountPlans.find(ap => ap.id === id)?.name || 'Desconhecido';
  };

  const getCostCenterName = (id: string) => {
    return costCenters.find(cc => cc.id === id)?.name || 'Desconhecido';
  };
