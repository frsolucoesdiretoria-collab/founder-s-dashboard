// OverviewPJ - Visão Geral Pessoa Jurídica
...
export function OverviewPJ() {
  ...
  const operationalMargin = totals.income > 0 ? ((totals.balance / totals.income) * 100) : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${operationalMargin >= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {BANK_ACCOUNTS.filter(ba => ba.entity === 'PJ').map((account) => (
              <div
                key={account.id}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Métricas importantes para PJ</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 rounded-lg border">
              <p className="text-sm text-muted-foreground">Burn Rate Mensal</p>
              <p className="text-2xl font-bold text-orange-600">
                {formatCurrency(totals.expenses)}
              </p>
            </div>
            <div className="p-4 rounded-lg border">
              <p className="text-sm text-muted-foreground">Runway (meses)</p>
              <p className="text-2xl font-bold text-blue-600">
                {totals.expenses > 0 
                  ? ((BANK_ACCOUNTS.filter(ba => ba.entity === 'PJ').reduce((sum, ba) => sum + ba.balance, 0)) / totals.expenses).toFixed(1)
                  : '∞'
                }
              </p>
            </div>
